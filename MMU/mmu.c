...
void allocate_memory(list_t * freelist, list_t * alloclist, int pid, int blocksize, int policy) {
    ...
    int blockend = blocknode->blk->end; // placeholder
  
    // 2. if so, remove it and go to #3, if not print ""Error: Memory Allocation <blocksize> blocks\n""
    if (!blocknode){printf("Error: Memory Allocation %d blocks\n", blocksize); return; }
    else { 
      if (!prev){freelist->head = freelist->head->next;}
      else{prev->next = blocknode->next; }
    }
  
    blocknode->blk->pid = pid; // 3. set the blk.pid = pid
    blocknode->blk->end = blocknode->blk->start + blocksize - 1;// 4. set the blk.end = blk->start + blocksize - 1
    list_add_ascending_by_address(alloclist, blocknode->blk); // 5. add the blk to the ALLOC_LIST in ascending order by address.
    // 6. Deal with the remaining left over memory (fragment).
    if (blocknode->blk->end != blockend){
      block_t *fragment = (block_t*)malloc(sizeof(block_t)); //     a. dynamically allocate a new block_t called fragment [use malloc]      
      fragment->pid = 0; //     b. set the fragment->pid = 0  
      fragment->start = blocknode->blk->end + 1; //     c. set the fragment->start = the blk.end + 1      
      fragment->end = blockend; //     d. set the fragment->end = original blk.end before you changed it in #4
      ...
      else if (policy == 3){ list_add_descending_by_blocksize(freelist, fragment); }
    }
...
void deallocate_memory(list_t * alloclist, list_t * freelist, int pid, int policy) { 
    ...
    if (!curr){ printf("Error: Can't locate Memory Used by PID: %d\n", pid); return; }
    ...
    if (policy == 1){ list_add_to_back(freelist, deallocateblock); }
    else if (policy == 2){ list_add_ascending_by_blocksize(freelist, deallocateblock); }
    else if (policy == 3){ list_add_descending_by_blocksize(freelist, deallocateblock); }
}
